...
describe('TUI config imports', () => {
    ...
    it('synchronizes Chalk and config error state after reloading settings', () => {
        const originalLevel = chalk.level;

        try {
            const loaded = applyTuiLoadedSettings(
                { ...DEFAULT_SETTINGS, colorLevel: 1 },
                null
            );

            expect(loaded.settings.colorLevel).toBe(1);
            expect(loaded.configLoadError).toBeNull();
