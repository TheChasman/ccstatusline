...
export function applyTuiLoadedSettings(
    loadedSettings: Settings,
    configLoadError: string | null
): { settings: Settings; configLoadError: string | null } {
...
export const App: React.FC = () => {
    ...
    useEffect(() => {
        ...
        void loadSettings().then((loadedSettings) => {
            ...
            const loadedState = applyTuiLoadedSettings(loadedSettings, getConfigLoadError());
            setSettings(loadedState.settings);
            setOriginalSettings(cloneSettings(loadedState.settings));
            setConfigLoadError(loadedState.configLoadError);
        });
        void isInstalled()
        ...
        setConfirmDialog({
            ...
            action: async () => {
                try {
                    ...
                    const loadedState = applyTuiLoadedSettings(reloaded, getConfigLoadError());
                    setSettings(loadedState.settings);
                    setOriginalSettings(cloneSettings(loadedState.settings));
                    setConfigLoadError(loadedState.configLoadError);
                    setHasChanges(false);
                    ...
                    setScreen('install');
                    setFlashMessage({
                        text: '✗ Reset failed; existing settings were preserved',
                        color: 'red'
                    });
                } finally {
                    setConfirmDialog(null);
