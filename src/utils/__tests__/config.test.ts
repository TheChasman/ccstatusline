...
let backupAndResetToDefaults: () => Promise<string>;
...
describe('config utilities', () => {
    beforeAll(async () => {
        ...
        backupAndResetToDefaults = configModule.backupAndResetToDefaults;
    });
    ...
    it('backs up settings, resets defaults, and removes managed hooks', async () => {
        const { settingsPath, backupPath, configDir } = getSettingsPaths();
        const claudeSettingsPath = path.join(getClaudeConfigDir(), 'settings.json');
        fs.mkdirSync(configDir, { recursive: true });
        ...
        fs.writeFileSync(claudeSettingsPath, JSON.stringify({
            hooks: {
                UserPromptSubmit: [{
                    ...
                    hooks: [{ type: 'command', command: 'ccstatusline --hook' }]
                }]
            }
        }), 'utf-8');

        const returnedBackupPath = await backupAndResetToDefaults();

        expect(returnedBackupPath).toBe(backupPath);
        expect(JSON.parse(fs.readFileSync(backupPath, 'utf-8'))).toEqual(
            expect.objectContaining({ globalBold: true })
        );
        expect(JSON.parse(fs.readFileSync(settingsPath, 'utf-8'))).toEqual(
            expect.objectContaining({ globalBold: DEFAULT_SETTINGS.globalBold })
        );
        expect(JSON.parse(fs.readFileSync(claudeSettingsPath, 'utf-8'))).not.toHaveProperty('hooks');
    });

    it('does not overwrite settings when the backup cannot be written', async () => {
        const { settingsPath, backupPath, configDir } = getSettingsPaths();
        fs.mkdirSync(configDir, { recursive: true });
        ...
        await expect(backupAndResetToDefaults()).rejects.toThrow();
