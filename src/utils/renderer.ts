...
function renderPowerlineStatusLine(
    ...
    for (let i = 0; i < filteredWidgets.length; i++) {
        ...
        if (widgetText) {
            ...
            // Apply theme colors first so partial dynamic styles inherit any
            // foreground/background values they do not explicitly replace.
            ...
            } else if (!widget.merge) {
                // Still need to increment color index for non-merged widgets even if no theme
                widgetColorIndex++;
            }

            if (dynamicColors) {
                fgColor = dynamicColors.color ?? fgColor;
                bgColor = dynamicColors.backgroundColor ?? bgColor;
                isBold = dynamicColors.bold ?? isBold;
            }
    ...
    for (let i = 0; i < widgetElements.length; i++) {
        ...
        const shouldBold = (settings.globalBold) || (widget.isBold ?? widget.widget.bold);
        ...
        if (widget.type === 'separator') {
            ...
            if (settings.inheritSeparatorColors && !widget.color && !widget.backgroundColor) {
                ...
                if (prevWidget) {
                    ...
                    if (dynamicColors) {
                        widgetColor = dynamicColors.color ?? widgetColor;
                        separatorBg = dynamicColors.backgroundColor ?? prevWidget.backgroundColor;
                        separatorBold = dynamicColors.bold ?? prevWidget.bold;
                    } else {
        ...
        try {
            ...
            if (widgetText) {
                ...
                if (widget.type === 'custom-command' && widget.preserveColors) {
                    ...
                    elements.push({
                        content: applyColorsWithOverride(widgetText, widgetColor, widgetBackgroundColor, widgetBold, widget.dim),
                        type: widget.type,
                        widget
                    });
                }
        ...
        if (shouldAddSeparator) {
            ...
            if (settings.inheritSeparatorColors && index > 0) {
                ...
                if (prevElem?.widget) {
                    ...
                    const coloredSep = applyColorsWithOverride(defaultSep, widgetColor, bgColor, isBold, prevElem.widget.dim);
